...
resource "aws_iam_policy" "task_role_ssh_exec" {
 name        = "${var.service-config.name}_ssh_exec"
 policy = jsonencode({
  Version = "2012-10-17"
  Statement = [
    {
...
resource "aws_iam_role_policy_attachment" "task_role_ssh_exec_policy" {
  role       = "${aws_iam_role.ecs_task_role.name}"
  policy_arn = "${aws_iam_policy.task_role_ssh_exec.arn}"
}